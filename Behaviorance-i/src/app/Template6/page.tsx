...
export default function TemplatePage() {
  ...
  useEffect(() => {
    const fetchQuestions = async () => {
      try {
        const res = await fetch('/api/Template6/questions');
        if (!res.ok) {
  ...
  //   try {
  //     const res = await fetch('/api/Template6/Submit', {
  //       method: 'POST',
  ...
  if (answers.length !== 11 || answers.includes("")) {
    alert("Please answer all 11 questions.");
    return;
  }
