// // app/api/Template5/Submit/route.ts
...
//     // Dynamically add all 10 answers
//     for (let i = 0; i < 10; i++) {
//       request.input(`ans${i + 1}`, mssql.NVarChar, answers[i]);
//     }

//     await request.query(`
//       INSERT INTO Domain5_Answer (
...
//   .input('Template_number', mssql.Int, 5) // ← replace with dynamic number
//   .query(`
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // Bind 10 answers and scores for Template 5
    for (let i = 1; i <= 10; i++) {
    ...
    // Mark submission in User_Template_Status
    await conn.request()
      .input('User_id', mssql.Int, userId)
      .input('Template_number', mssql.Int, 5)
      .query(`
        INSERT INTO User_Template_Status (User_id, Template_number, Domain_name)
        SELECT @User_id, @Template_number, Domain_Name
        FROM SurveyDomains
