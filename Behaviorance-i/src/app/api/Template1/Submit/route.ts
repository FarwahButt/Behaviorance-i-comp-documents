// // app/api/Template1/Submit/route.ts
...
//   .input('Template_number', mssql.Int, 1) // ← replace with dynamic number
//   .query(`
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // Bind all 15 answer texts and scores
    for (let i = 1; i <= 15; i++) {
    ...
    // Insert into User_Template_Status
    await conn.request()
      .input('User_id', mssql.Int, userId)
      .input('Template_number', mssql.Int, 1)
      .query(`
        INSERT INTO User_Template_Status (User_id, Template_number, Domain_name)
        SELECT @User_id, @Template_number, Domain_Name
        FROM SurveyDomains
