...
export default function TemplatePage() {
  ...
  useEffect(() => {
    const fetchQuestions = async () => {
      try {
        const res = await fetch('/api/Template12/questions');
        if (!res.ok) {
  ...
  //   try {
  //     const res = await fetch('/api/Template12/Submit', {
  //       method: 'POST',
  ...
  if (answers.length !== 14 || answers.includes("")) {
    alert("Please answer all 14 questions.");
    return;
  }
